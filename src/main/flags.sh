...
if [[ -n "${SEMGREP_NIX_BUILD-}" ]]; then
    ...
    macosx)
        ...
        CCLIB=(
             ...
             "$(pkg-config libzstd --variable libdir)/libzstd.a"
             "$(pkg-config libidn2 --variable libdir)/libidn2.a"
             "$(pkg-config zlib --variable libdir)/libz.a"
             "$(pkg-config libcurl --variable libdir)/libcurl.a"
             "$(opam var prefix)/lib/curl/libcurl-helper.a"
        )
